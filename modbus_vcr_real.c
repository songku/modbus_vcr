...
unsigned char *seq1 = NULL, *seq2 = NULL;
struct request_response_item *maqicurr = NULL;
int flag = 0, flag_1 = 0;
// INPUT PORT
char cNome[20];
char *ptr = NULL;
unsigned short maqi_port;
...
struct modbus_ident
{
    ...
    u_int16 slave_port;            // L4_dst; // tcp dest, slave port, should be 21502
};
...
static EC_THREAD_FUNC(modbus_accept_thread)
{
    ...
    LOOP
    {
        ...
        if (poll_fd.revents & POLLIN)
        {
            ...
            connection->port[MODBUS_SERVER] = htons(maqi_port);
...
static int modbus_read(struct modbus_connection *connection, struct packet_object *po)
{
    ...
    if (len < 0)
    { // going out on a limb here, changing from <= 0 to <0 KRW
        if (errno == EAGAIN)
        {
...
static int modbus_vcr_init(void *modbus_vcr)
{
    ...
    ui_input("Please input a modbus port:", cNome, sizeof(cNome) - 1, NULL);
    // hook_add(HOOK_FILTER, &record_modbus); // we might want HOOK_PRE_FORWARD?
    maqi_port = (unsigned short)strtoul(cNome, &ptr, 0);
...
static int modbus_vcr_is_modbus(struct packet_object *po)
{
    ...
    if (po->L4.proto != NL_TYPE_TCP)
        return 0;
    // bug here, whoops
    // if (ntohs(po->L4.dst) == (unsigned short)21502 || ntohs(po->L4.src) == (unsigned short)21502)
    //     return 1;
    if (ntohs(po->L4.dst) == maqi_port || ntohs(po->L4.src) == maqi_port)
        return 1;
...
// or a master packet :(.
// I'm going to decide that whichever has tcp/21502 is the slave packet
static size_t modbus_vcr_create_ident(void **i, struct packet_object *po)
{
    struct modbus_ident *ident;
    unsigned short mbport = maqi_port;
...
static int find_request_response_pair(struct request_response_item **output_request_response_item, u_char *requestdata, size_t requestlen, struct request_response_item *head_request_response_item)
{
...
static int find_response(struct request_response_item **output_request_response_item, u_char *responsedata, size_t responselen, struct request_response_item *head_request_response_item)
{
    struct request_response_item *curr = head_request_response_item;
    // walk through the list
...
static void modbus_handle_request(struct packet_object *po)
{
    ...
    if (0 == ((session_data_t *)s->data)->done_recording)
    {
        ...
        if (timeval_now.tv_sec > (timeval_sessionstart.tv_sec + 15))
        {
        ...
        else
        {
            ...
            if (PACKET->DATA.len != 0)
            {
                if (is_request(s, PACKET))
                {
                    ...
                    memcpy(seq1, PACKET->DATA.data, 2);
                    ...
                    SAFE_FREE(temp_request_response_pair->requestData);
                ...
                else
                {
                    if (flag)
                    {
                        ...
                        SAFE_CALLOC(seq2, 2, sizeof(u_char));
                        memcpy(seq2, PACKET->DATA.data, 2);
                        if (memcmp(seq1, seq2, 2))
                        {
                        ...
                        SAFE_FREE(temp_request_response_pair->responseData);
                        ...
                        ((session_data_t *)s->data)->current_request_response_item = ((session_data_t *)s->data)->current_request_response_item->next;
                    }
                }
            }
        }
    ...
    {
        // Print recorded data
        // printf("++++++++++++++++++++Print recorded data!+++++++++++++++++++++++\n");
        // maqicurr = ((session_data_t *)s->data)->head_request_response_item;
        // while (maqicurr != NULL)
        // {
        //     printf("The data is the request data that we have logged:\n");
        //     // printf("%x ", maqicurr->mrrp.requestData);
        //     for (i = 0; i < maqicurr->mrrp.requestDataLen; i++)
        //     {
        //         printf("%02x ", maqicurr->mrrp.requestData[i]);
        //     }
        //     printf("\n");
        //     printf("The data is the response data that we have logged:\n");
        //     for (i = 0; i < maqicurr->mrrp.responseDataLen; i++)
        //     {
        //         printf("%02x ", maqicurr->mrrp.responseData[i]);
        //     }
        //     printf("\n");
        //     maqicurr = maqicurr->next; // potential null pointer dereference
        // }
        ...
        if (is_request(s, PACKET))
        {
            ...
            if (PACKET->DATA.len != 0)
            {
                ...
                SAFE_CALLOC(seq1, 2, sizeof(u_char));
                memcpy(seq1, PACKET->DATA.data, 2);
                flag_1 = find_request_response_pair(&((session_data_t *)s->data)->current_request_response_item, PACKET->DATA.data, PACKET->DATA.len, ((session_data_t *)s->data)->head_request_response_item);
                flag = 1;
            }
        }
        else
        { /* is_request(s, PACKET) */
            ...
            if (PACKET->DATA.len != 0 && flag_1 && flag)
            {
                ...
                if (memcmp(seq1, seq2, 2))
                {
                    ...
                    if (!find_response(&((session_data_t *)s->data)->current_request_response_item, PACKET->DATA.data, PACKET->DATA.len, ((session_data_t *)s->data)->head_request_response_item))
                        return;
                    else
                        printf("Serial number mismatch!\n");
                }
                SAFE_FREE(seq1);
                SAFE_FREE(seq2);
                SAFE_CALLOC(seq1, 4, sizeof(u_char));
                SAFE_CALLOC(seq2, 4, sizeof(u_char));
                memcpy(seq1, &PACKET->DATA.data[4], 4);
                memcpy(seq2, &((session_data_t *)s->data)->current_request_response_item->mrrp.responseData[4], 4);
                if (memcmp(seq1, seq2, 4))
                {
                    if (!find_response(&((session_data_t *)s->data)->current_request_response_item, PACKET->DATA.data, PACKET->DATA.len, ((session_data_t *)s->data)->head_request_response_item))
                        return;
                    else
 ...
 * This will accept incoming connections on TCP/21502 */
...
static int modbus_bind_wrapper(void)
{
    ...
    listen(main_fd, 100);
    USER_MSG("modbus_vcr plugin: bind %s on %d\n, bind_port", cNome, bind_port);
...
static int modbus_insert_redirect(u_int16 dport)
{
    ...
    command = strdup(EC_GBL_CONF->redir_command_on);
    str_replace(&command, "%iface", EC_GBL_OPTIONS->iface);
    str_replace(&command, "%port", cNome);
...
static int modbus_remove_redirect(u_int16 dport)
{
    ...
    if (EC_GBL_CONF->redir_command_on == NULL)
        return -E_FATAL;
    snprintf(asc_dport, 16, "%u", dport);
    command = strdup(EC_GBL_CONF->redir_command_off);
    str_replace(&command, "%iface", EC_GBL_OPTIONS->iface);
    str_replace(&command, "%port", cNome);
    str_replace(&command, "%rport", asc_dport);
#if defined(OS_DARWIN) || defined(OS_BSD)
